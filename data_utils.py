...
def make_argument_vocab(train_file, dev_file, test_file,  output_path, freq_lower_bound=0, quiet=False,
                        use_lower_bound=False, unify_pred=False, use_nomore_tag=False):
    argument_data = []
...
def make_pred_vocab(train_file, dev_file, test_file, output_path, freq_lower_bound=0, quiet=False,
...
def shrink_pretrained_embedding(train_file, dev_file, test_file, pretrained_file, pretrained_emb_size, output_path,
                                quiet=False):
    word_set = set()
    with open(train_file, 'r') as f:
        data = f.readlines()
...
def make_dataset_input(dataset_file, output_path, quiet=False, random_error_prob=0.0, deprel_vocab=None,
                       unify_pred=False, predicate_recog_data=None, use_golden_syn=False, use_nomore_tag=False,
                       pickle_dump_path=None):  # , word_filter=None
        ...
        for sidx in tqdm(range(len(origin_data))):
            ...
            for i in range(len(sentence)):
                if sentence[i][12] == 'Y':
                    ...
                    if unify_pred:
                        output_block.append(
                            [str(sentence_idx), str(predicate_idx), str(len(sentence) + 1), '1', '1', '0', '<DUMMY>',
                             '<DUMMY>', '<DUMMY>', str(int(sentence[i][0]) + 1), str(int(sentence[i][0]) + 1),
                             '<DUMMY>', sentence[i][13].split('.')[1]])
                    ...
                    for j in range(len(sentence)):
                        ...
                        if unify_pred:
                            output_block.append(
                                [str(sentence_idx), str(predicate_idx), str(len(sentence) + 1), str(int(ID) + 1),
                                 str(int(ID) + 1), str(IS_PRED), word, lemma, pos, str(int(head) + 1),
                                 str(int(head) + 1), deprel, tag])
                        else:
                            output_block.append(
                                [str(sentence_idx), str(predicate_idx), str(len(sentence)), ID, ID, str(IS_PRED), word,
                                 lemma, pos, head, head, deprel, tag])
    ...
    if pickle_dump_path is not None:
        dump_data = {'predicate_sum': predicate_sum, 'target_sum': target_sum, 'out_of_target_sum': 0,
                     'argument_sum': argument_sum, 'out_of_pruning_sum': 0, 'input_data': output_data, 'K': 0}
